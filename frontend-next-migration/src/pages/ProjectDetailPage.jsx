...
function ProjectDetailContent() {
    ...
    if (isLoading) return <div className="flex min-h-screen items-center justify-center"><Loader2 className="h-12 w-12 animate-spin text-acm-blue" /></div>;

    if (isError || !project) return (
        <div className="flex min-h-screen flex-col items-center justify-center px-4 text-center">
            <div className="mb-6 rounded-full bg-red-500/10 p-4"><AlertCircle className="h-12 w-12 text-red-500" /></div>
            <h1 className="text-3xl font-bold tracking-tight">Project Not Found</h1>
            <p className="mt-2 text-muted-foreground max-w-sm">The project you're looking for doesn't exist or has been removed.</p>
            <Button onClick={() => router.push("/projects")} className="mt-8 bg-acm-blue hover:bg-acm-blue-dark">Back to Projects</Button>
        </div>
    );
        ...
        <div className="min-h-screen bg-background/50 pb-20">
            <div className="border-b border-border/50 bg-background/80 backdrop-blur-md sticky top-0 z-30">
                <div className="container mx-auto px-4 py-4 flex items-center justify-between">
                    <Button variant="ghost" size="sm" onClick={() => router.push("/projects")} className="text-muted-foreground hover:text-foreground -ml-2">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {canEdit && (
                            <>
                                <Button variant="outline" size="icon" onClick={() => router.push(`/projects/${projectId}/edit`)} className="h-9 w-9 rounded-xl border-border/50">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <div className="lg:col-span-2 space-y-10">
                        <div className="space-y-6">
                            ...
                            <div>
                                <h1 className="text-4xl sm:text-5xl lg:text-7xl font-black tracking-tight mb-8 leading-[1.1] text-gradient-acm italic">
                                    {project.title}
                                </h1>
                                <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground font-bold uppercase tracking-widest">
                                    <div className="flex items-center gap-2 bg-acm-blue/5 px-4 py-2 rounded-xl border border-acm-blue/10">
                                        <Calendar className="h-4 w-4 text-acm-blue" />
                                        <span>{project.createdAt ? format(new Date(project.createdAt), "MMMM d, yyyy") : "Recently"}</span>
                                    </div>
                                    {project.contributors?.length > 0 && (
                                        <div className="flex items-center gap-2 bg-acm-blue/5 px-4 py-2 rounded-xl border border-acm-blue/10">
                                            <Users className="h-4 w-4 text-acm-blue" />
                            ...
                            <div className="space-y-6">
                                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-acm-blue flex items-center gap-2">
                                    <div className="w-8 h-px bg-acm-blue/30" /> Project Brief
                                </h3>
                                <p className="text-foreground/90 whitespace-pre-wrap leading-relaxed text-xl font-medium italic">
                                    {project.description}
                                </p>
                            </div>
                        </div>
                        ...
                        {project.assets?.length > 0 && (
                            <div className="space-y-8 pt-8">
                                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-acm-blue flex items-center gap-2 text-primary">
                                    <div className="w-8 h-px bg-acm-blue/30" /> Media Gallery
                                </h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {project.assets.map((asset) => (
                                        <a key={asset.id} href={asset.url} target="_blank" rel="noopener noreferrer"
                                            className="group relative aspect-video overflow-hidden rounded-[2rem] border border-border/40 bg-card/40 backdrop-blur-xl transition-all hover:border-acm-blue/50 hover:shadow-acm-glow">
                                            {asset.contentType?.startsWith("image/") ? (
                                                <>
                                                    <img src={asset.url} alt={asset.filename} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                                ...
                                                <div className="flex h-full w-full flex-col items-center justify-center p-8 text-center bg-muted/20">
                                                    <div className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center mb-4 border border-white/10 group-hover:bg-acm-blue/10 group-hover:border-acm-blue/30 transition-all">
                                                        <Code2 className="h-8 w-8 text-acm-blue" />
                                                    </div>
                                                    <span className="max-w-[200px] truncate text-sm font-bold uppercase tracking-widest">{asset.filename}</span>
                                                </div>
                                            )}
                                        </a>
                                    ))}
                    ...
                    <div className="space-y-8">
                            ...
                            <Card className="border-border/50 bg-card/50 backdrop-blur-sm rounded-3xl">
                                <CardHeader>
                                    <CardTitle className="text-lg flex items-center gap-2">
                                        <Code2 className="h-5 w-5 text-acm-blue" /> Built With
                                    </CardTitle>
                                </CardHeader>
                                <CardContent>
                                    <div className="flex flex-wrap gap-2">
                                        {project.techStack.map((tech, index) => (
                                            <Badge key={index} variant="secondary" className="bg-acm-blue/10 text-acm-blue border-none px-3 py-1 rounded-xl text-xs font-bold">{tech}</Badge>
                                        ))}
                                    </div>
                                </CardContent>
                            </Card>
                        ...
                        <Card className="border-border/50 bg-card/50 backdrop-blur-sm rounded-3xl">
                            <CardHeader>
                                <CardTitle className="text-lg flex items-center gap-2"><Globe className="h-5 w-5 text-acm-blue" /> Resources</CardTitle>
                            </CardHeader>
                            ...
                            <CardContent className="p-6">
                                <dl className="space-y-4">
                                    ...
                                    <div className="flex items-center justify-between text-sm">
                                        ...
                                        <dd className="font-bold">{project.updatedAt ? format(new Date(project.updatedAt), "MMM d, yyyy") : "Recent"}</dd>
                                    </div>
