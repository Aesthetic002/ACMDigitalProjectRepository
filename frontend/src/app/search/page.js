                        ...
                        <form onSubmit={handleSearch} className="group relative mx-auto max-w-2xl">
                            <div className="flex flex-col gap-3 sm:flex-row">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <Button type="submit" className="h-14 rounded-2xl bg-acm-blue px-8 text-lg font-bold hover:bg-acm-blue-dark">
                                        SEARCH
                                    </Button>
                                </div>
                            </div>
                            ...
                            {showFilters && (
                                <div className="mt-4 overflow-hidden rounded-3xl border border-border/50 bg-card p-6 text-left shadow-xl animate-in fade-in slide-in-from-top-4 duration-300">
                                    ...
                                    <div className="grid gap-6 sm:grid-cols-2">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            >
                                                <SelectTrigger className="h-10 border-border/50 bg-muted/30 rounded-xl">
                ...
                {query ? (
                    <div className="mx-auto max-w-5xl">
                        ...
                        {isLoading ? (
                            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                                {[1, 2, 3].map((i) => (
                                    <div key={i} className="h-[400px] animate-pulse rounded-3xl bg-muted/40 border border-border/50" />
                                ))}
                            </div>
                        ) : results.length === 0 ? (
                            <div className="flex min-h-[400px] flex-col items-center justify-center rounded-[3rem] border border-dashed border-border p-12 text-center">
                                ...
                                <p className="mt-2 text-muted-foreground max-w-xs mx-auto">
                                    We couldn't find any projects matching your criteria. Try broad keywords or removing filters.
                                </p>
                                <div className="mt-8 flex gap-3">
                                    <Button variant="outline" onClick={() => router.push('/projects')} className="rounded-2xl px-6">
                                        Browse All
                                    </Button>
                                    <Button variant="secondary" onClick={() => { setSearchInput(''); router.push('/search') }} className="rounded-2xl px-6">
                                        Clear Search
                                    </Button>
                                </div>
                            </div>
                    ...
                    <div className="mx-auto max-w-4xl text-center py-20">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            {[
                                { label: 'Popular Web Apps', query: 'Web' },
                                { label: 'Data Science', query: 'Data' },
                                { label: 'Cloud Solutions', query: 'Cloud' }
                            ].map((item) => (
                                <Button
                                    key={item.label}
                                    variant="outline"
