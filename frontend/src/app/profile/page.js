...
export default function ProfilePage() {
    ...
    const getStatusConfig = (status) => {
        switch (status) {
            case 'approved':
                return { icon: CheckCircle, color: 'text-emerald-500', bg: 'bg-emerald-500/10', label: 'Verified' };
            case 'pending':
    ...
    return (
        <div className="min-h-screen bg-background/50 pb-20">
            ...
            <section className="relative overflow-hidden bg-slate-950 py-20 border-b border-border/50">
                ...
                <div className="container relative mx-auto px-4">
                    <div className="flex flex-col md:flex-row items-center gap-8 md:items-end">
                        <Avatar className="h-32 w-32 rounded-[2.5rem] border-4 border-background shadow-2xl">
                            ...
                            <AvatarFallback className="bg-acm-blue text-white text-4xl font-black rounded-[2.5rem]">
                            ...
                            <div className="space-y-1">
                                    ...
                                    {isEditing ? (
                                        <div className="flex items-center gap-2">
                                            <input
                                                ...
                                                className="bg-muted px-4 py-2 rounded-xl border border-acm-blue font-bold text-2xl focus:outline-none focus:ring-2 focus:ring-acm-blue/20"
                                                autoFocus
                                            />
                                ...
                                <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-slate-400 text-sm font-medium">
                                    <span className="flex items-center gap-2">
                                        <Mail className="h-4 w-4 text-acm-blue/50" />
                                        {user?.email}
                                    </span>
                                    <span className="flex items-center gap-2">
                        ...
                        <div className="flex gap-3">
                            <Button variant="outline" onClick={logout} className="rounded-2xl border-red-500/20 text-red-500 hover:bg-red-500/10 font-bold h-12 px-6">
            ...
            <div className="container mx-auto px-4 py-12">
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    <div className="space-y-6">
                        <Card className="rounded-[2rem] border-border/50 bg-card/50 overflow-hidden shadow-xl">
                        ...
                        <Button asChild className="w-full h-14 rounded-2xl bg-acm-blue hover:bg-acm-blue-dark shadow-acm-glow font-black tracking-widest">
                            <Link href="/submit">
                                <Plus className="mr-2 h-5 w-5" /> NEW SUBMISSION
                            </Link>
                        </Button>
                    </div>
                    ...
                    <div className="lg:col-span-3">
                        ...
                        {projectsLoading ? (
                            ...
                            <Card className="rounded-[3rem] border-dashed border-2 border-border/50 bg-muted/5 p-20 text-center">
                                <div className="mb-6 rounded-full bg-muted p-4 inline-flex">
                                    <FolderOpen className="h-12 w-12 text-muted-foreground" />
                                </div>
                                <h3 className="text-xl font-bold mb-2">No active projects found.</h3>
                                <p className="text-muted-foreground mb-8 max-w-xs mx-auto text-sm">You haven't submitted any projects to the digital repository yet.</p>
                                <Button asChild className="rounded-2xl h-12 px-8 bg-acm-blue hover:bg-acm-blue-dark">
                                    <Link href="/submit">Start your first contribution</Link>
                                </Button>
                            </Card>
                        ) : (
                                                ...
                                                <div className="flex items-start justify-between gap-4 mb-4">
                                                    ...
                                                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <Button variant="ghost" size="icon" asChild className="h-8 w-8 rounded-lg hover:bg-acm-blue/10">
                                                            <Link href={`/projects/${project.id}/edit`}><Edit3 className="h-4 w-4 text-acm-blue" /></Link>
                                                        </Button>
                                                        <Button variant="ghost" size="icon" asChild className="h-8 w-8 rounded-lg hover:bg-acm-blue/10">
                                                            <Link href={`/projects/${project.id}`}><ExternalLink className="h-4 w-4 text-acm-blue" /></Link>
                                                        </Button>
                                                    </div>
                                                </div>
                                                <h4 className="text-lg font-bold truncate group-hover:text-acm-blue transition-colors mb-2 uppercase tracking-tight">
                                                    {project.title}
                                                </h4>
