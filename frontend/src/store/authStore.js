    ...
    (set, get) => ({
      ...
      initAuth: () => {
        ...
        if (currentState.isAuthenticated && currentState.user) {
          console.log(
            "[Mock Auth] Restored session for:",
      ...
      login: async (email, password, role = "contributor") => {
        ...
        if (existingUser) {
          ...
          toast.success(
            existingUser.role === "admin" ? "Welcome, Admin!" : "Welcome back!",
          );
      ...
      loginAsDemo: (role = "admin") => {
        const demoUser = {
          ...
          email:
            role === "admin" ? "admin@acm-demo.local" : "member@acm-demo.local",
...
console.log(
  "%c[MOCK AUTH] Running in mock authentication mode - no Firebase required",
