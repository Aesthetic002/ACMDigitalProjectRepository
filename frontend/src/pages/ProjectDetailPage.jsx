...
function ProjectDetailContent() {
    ...
    if (isError || !project) return (
        <div className="flex min-h-screen flex-col items-center justify-center px-4 text-center">
            ...
            <Button onClick={() => navigate("/projects")} className="mt-8 bg-acm-blue hover:bg-acm-blue-dark">Back to Projects</Button>
        </div>
    );
        ...
        <div className="min-h-screen bg-background/50 pb-20">
            <div className="border-b border-border/50 bg-background/80 backdrop-blur-md sticky top-0 z-30">
                <div className="container mx-auto px-4 py-4 flex items-center justify-between">
                    <Button variant="ghost" size="sm" onClick={() => navigate("/projects")} className="text-muted-foreground hover:text-foreground -ml-2">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {canEdit && (
                            <>
                                <Button variant="outline" size="icon" onClick={() => navigate(`/projects/${projectId}/edit`)} className="h-9 w-9 rounded-xl border-border/50">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <div className="lg:col-span-2 space-y-10">
                        <div className="space-y-6">
                            ...
                            <div>
                                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight mb-6">{project.title}</h1>
                            ...
                            <div className="prose prose-invert max-w-none">
                                ...
                                <p className="text-muted-foreground whitespace-pre-wrap leading-loose text-lg">{project.description}</p>
                            </div>
                        </div>
                                    ...
                                    {project.assets.map((asset) => (
                                        <a key={asset.id} href={asset.url} target="_blank" rel="noopener noreferrer"
                                            ...
                                            {asset.contentType?.startsWith("image/") ? (
                                                <>
                                                    <img src={asset.url} alt={asset.filename} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
                                                    <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                                                        <div className="rounded-full bg-white/10 p-4 backdrop-blur-md"><ExternalLink className="h-6 w-6 text-white" /></div>
                                                    </div>
                                                </>
                                            ) : (
