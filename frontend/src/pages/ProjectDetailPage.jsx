...
export function ProjectDetailContent({ backUrl = "/projects" }) {
  ...
  const { user, isAuthenticated, hasSeenRoleDialog, setGuestRole } = useAuthStore();
  
  const [showRoleDialog, setShowRoleDialog] = useState(false);

  // Show role dialog on first project visit for non-authenticated users
  useEffect(() => {
  ...
  const handleRoleSelect = (role) => {
    ...
    toast.success(`Welcome! You're browsing as a ${role === 'viewer' ? 'Viewer' : 'Contributor'}.`);
  };
  ...
  const canEdit =
    isOwner ||
    project?.contributors?.includes(user?.uid) ||
    user?.role === "admin";
  ...
  if (isError || !project)
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-4 text-center bg-slate-950">
        <div className="mb-6 rounded-full bg-red-500/10 p-4 shadow-inner border border-red-500/20">
        ...
        <p className="mt-2 text-muted-foreground max-w-sm font-medium">
          The project you're looking for doesn't exist or has been removed from
          the archive.
        </p>
        <Button
          onClick={() => navigate(backUrl)}
          className="mt-10 bg-white text-slate-950 hover:bg-slate-200 font-black px-8 py-6 rounded-xl uppercase italic tracking-widest transition-all shadow-xl"
        >
          Back to Collection
        </Button>
      </div>
      ...
      <div className="border-b border-border/50 bg-slate-950/80 backdrop-blur-xl sticky top-0 z-30 border-t border-t-white/5">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <Button
            ...
            size="sm"
            onClick={() => navigate(backUrl)}
            className="text-muted-foreground hover:text-white font-black uppercase tracking-widest text-[10px] italic -ml-2"
          >
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              size="icon"
              onClick={handleShare}
              className="h-10 w-10 rounded-xl border-border/50 bg-white/5 hover:bg-white/10 transition-all"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-12">
            <div className="space-y-8">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Badge
                  variant="outline"
                  className={`${statusInfo.bg} ${statusInfo.color} ${statusInfo.border} gap-1.5 uppercase font-black tracking-widest text-[9px] px-3 py-1 italic bg-white/5`}
                >
                  <StatusIcon className="h-3.5 w-3.5" /> {statusInfo.label}
                </Badge>
              </div>
              <div>
                <h1 className="text-4xl sm:text-5xl lg:text-7xl font-black tracking-tighter text-white uppercase italic mb-8 leading-none">
                  {project.title}
                </h1>
                <div className="flex flex-wrap items-center gap-4 text-[10px] text-muted-foreground font-black uppercase tracking-[0.2em] italic">
                  <div className="flex items-center gap-2 bg-white/5 border border-white/5 px-4 py-2 rounded-xl backdrop-blur-md">
                    ...
                    <span>
                      ...
                      {parseDate(project.createdAt)
                        ? format(parseDate(project.createdAt), "MMM d, yyyy")
                        : "RECENTLY"}
                    </span>
                  </div>
                  {project.contributors?.length > 0 && (
                    <div className="flex items-center gap-2 bg-white/5 border border-white/5 px-4 py-2 rounded-xl backdrop-blur-md">
                      ...
                      <span>
                        {project.contributors.length} ACTIVE MEMBER
              ...
              <div className="space-y-6">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-acm-blue italic">
                  Technical Brief
                </h3>
                      ...
                      {asset.contentType?.startsWith("image/") ? (
                        <>
                          ...
                          <div className="absolute inset-0 flex items-center justify-center bg-black/60 opacity-0 transition-opacity duration-300 group-hover:opacity-100 backdrop-blur-sm">
                            <div className="rounded-2xl bg-white/10 p-5 backdrop-blur-md border border-white/20 shadow-2xl transition-transform duration-300 group-hover:scale-110">
                        ...
                        <div className="flex h-full w-full flex-col items-center justify-center p-8 text-center bg-gradient-to-br from-white/5 to-transparent">
                          ...
                          <span className="max-w-[180px] truncate text-[10px] font-black uppercase tracking-widest text-slate-400 italic mb-1">
                            {asset.filename}
                          </span>
                          <span className="text-[9px] font-black text-white/20 uppercase tracking-tighter italic">
                            External Resource
                          </span>
                        </div>
                      )}
          ...
          <div className="space-y-8 lg:sticky lg:top-24 h-fit">
                  ...
                  <div className="flex flex-wrap gap-2.5">
                    {project.techStack.map((tech, index) => (
                      <Badge
                        key={index}
                        variant="secondary"
                        className="bg-white/5 text-white/70 border border-white/5 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wide hover:bg-acm-blue/10 hover:text-acm-blue hover:border-acm-blue/20 transition-all italic"
                      >
                        {tech}
                      </Badge>
                    ))}
            ...
            <Card className="border-border/50 bg-card/40 backdrop-blur-xl rounded-[2.5rem] shadow-2xl overflow-hidden border-t-2 border-t-white/5">
              <CardHeader className="pb-4">
                <CardTitle className="text-xs font-black uppercase tracking-[0.3em] text-acm-blue italic flex items-center gap-3">
                  <Globe className="h-4 w-4" /> Endpoint Links
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {(project.githubUrl || project.repoUrl) && (
                  <Button
                    asChild
                    variant="outline"
                    className="w-full justify-between gap-3 rounded-2xl h-14 font-black uppercase tracking-widest text-[10px] italic bg-white/2 border-white/5 hover:border-acm-blue/50 hover:bg-acm-blue/5 hover:text-acm-blue transition-all group px-6"
                  >
                    <a
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Github className="h-5 w-5 opacity-40 group-hover:opacity-100 transition-opacity" />{" "}
                        Source Code
                      </div>
                ...
                {project.demoUrl && (
                  <Button
                    asChild
                    className="w-full justify-between gap-3 rounded-2xl h-14 font-black uppercase tracking-widest text-[10px] italic bg-acm-blue hover:bg-acm-blue-dark shadow-acm-glow transition-all group px-6 shadow-xl text-white"
                  >
                    <a
              ...
              <CardContent className="p-8">
                <dl className="space-y-6">
                  <div className="flex items-center justify-between">
                    <dt className="text-[10px] font-black uppercase tracking-widest text-muted-foreground italic">
                      Repository Status
                    </dt>
                    <dd className="font-black uppercase tracking-tighter text-[11px] flex items-center gap-2">
                      <div
                        className={`h-2 w-2 rounded-full shadow-lg ${project.status === "approved" ? "bg-emerald-500 shadow-emerald-500/20" : "bg-amber-500 shadow-amber-500/20"}`}
                      />
                      <span
                        className={
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <dd className="font-black text-[11px] text-white italic">
                      {parseDate(project.updatedAt)
                        ? format(parseDate(project.updatedAt), "MMM d, yyyy")
                        : "RECENT"}
                    </dd>
                  </div>
                  <div className="pt-6 border-t border-white/5">
                    <dt className="text-[9px] uppercase font-black text-muted-foreground/40 tracking-[0.3em] mb-3 leading-none italic">
                      Archive Identifier
                    </dt>
                    <dd className="font-mono text-[9px] break-all text-muted-foreground/30 bg-black/20 p-3 rounded-lg border border-white/2 select-all">
                      {project.id}
                    </dd>
                  </div>
                </dl>
